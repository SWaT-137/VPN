...
public class NotificationService {
    ...
    public NotificationService(final Service targetService) {
        ...
        String applicationName = "GEOCINTVPN"; // Имя по умолчанию, если не найдет
        ...
        notifcationBuilder.setContentTitle(applicationName + " Connecting...")
                // ИСПРАВЛЕНИЕ 1: Железобетонная иконка замка при старте
    ...
    public void setConnectedNotification(String remark, int iconResource) {
        if (mNotificationManager != null && notifcationBuilder != null) {
            if (isNotificationOnGoing) {
                // ИСПРАВЛЕНИЕ 2: Если пришла пустая иконка (0), используем системную иконку замка
                int safeIcon = (iconResource > 0) ? iconResource : android.R.drawable.ic_lock_lock;

                notifcationBuilder.setSmallIcon(safeIcon);
