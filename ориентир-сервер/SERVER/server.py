...
class SafeLogger:
    ...
    def _log(self, level, msg):
        try:
            ...
            getattr(self.logger, level)(msg.encode('ascii', 'ignore').decode('ascii'))
    ...
    def _save_state(self):
        try:
            ...
            with open(tmp, 'w') as f:
                json.dump({'used_ips': list(self.used_ips), 'next_ip': self.next_ip}, f)
...
class WintunWrapper:
    def __init__(self, dll_path="wintun.dll"):
        paths = [
            os.path.join(os.path.dirname(__file__), dll_path),
...
class ClientHandler:
    ...
    def _send_worker(self):
        ...
        with self.server.send_queues_lock:
            self.server.send_queues[self.assigned_ip] = q
        try:
            while self.running and self.server.running:
                try:
            ...
            with self.server.send_queues_lock:
                self.server.send_queues.pop(self.assigned_ip, None)
...
class VPNServer:
    ...
    def _listen(self, port):
        """✅ КРИТИЧЕСКИ: Метод _listen() для обработки подключений"""
        try:
            # Создаём SSL-контекст БЕЗ импорта из server.py (избегаем циклического импорта)
            ...
            # ✅ Wrap только после полной настройки контекста
            ...
            logger.error(f"Listen error {port}: {e}")
