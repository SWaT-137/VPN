...
SPLIT_RU_URL = "https://antifilter.download/list/subnet.lst" # откуда скачивается белый список РФ
SPLIT_RU_FILE = os.path.join(WORK_DIR, "split_ru.txt") # где записывается белый список ip
...
# базовые функции
def is_admin(): # проверка на запуск через администратора
...
def get_default_gateway(): # вызывет команду router - показывает ip роутера или сети
    try:
...
class VPNClientProtocol(asyncio.DatagramProtocol):
    ...
    def connection_made(self, transport): # срабатывает когда udp сокет готов
        self.transport = transport
        self.send_ip_request()

    def datagram_received(self, data, addr): # расшифровка, шифрование хеша,проверка времени пакета, чтение команд
    ...
    def send_ip_request(self): # упаковка команд и тд
...
#белый список разделение трафика
...
def add_bypass_routes(local_gw): # маршутизация
...
async def read_from_wintun(adapter, transport, protocol): # слушает виртуальный адаптер 
...
async def ip_request_loop(protocol):# автоперезапуск,если сервер ляжет
...
async def log_metrics(protocol):# метРики
    while True:
...
async def vpn_main():# банальный оркестратор,тоже будет описан в описании к проекту
...
def cleanup_vpn():#выключает адаптер при выходе
